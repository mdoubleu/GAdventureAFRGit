...
public class GameSceneBuyItem extends BaseScene{

	public Sprite addBuyPopup(GameSceneAfrica gameScene, String question, int price, Sprite image){
		...
		Sprite exitButton=new Sprite(0,0,rm.exitButton,vbom){
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY){
				...
				if(pSceneTouchEvent.isActionUp() && this.getTag()==3){
					GameSceneAfrica.getInstance().removePopup(0);
					GameSceneAfrica.getInstance().popupBox=GameSceneAfrica.getInstance().popupBoxSaved;
				}
				return true;
			}
		};
		...
		Sprite buyButtonShadow=new Sprite(0,0,rm.rectangleButton,vbom){
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY){
				...
				if(pSceneTouchEvent.isActionUp() && this.getTag()==3){
					if(rm.totalCoins>=(Integer)this.getUserData()){
						GameSceneAfrica.getInstance().updateHearts(1);
						GameSceneAfrica.getInstance().removePopup((Integer)this.getUserData());
						GameSceneAfrica.getInstance().popupBox=GameSceneAfrica.getInstance().popupBoxSaved;
					}else{
						this.setScale(1);
						//NOT ENOUGH MONEY
		...
		gameScene.registerTouchArea(buyButtonShadow);
		...
		gameScene.attachChild(rectangleOuter);
